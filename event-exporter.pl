...
my @fields = ();
foreach my $f(split('\'\s*,\s*\#?\s*\'', $config->{'default.EXPORT_FIELDS'})) {
    $f =~ s/^\'//; $f =~ s/\'$//;
    push @fields, $f;
}

my @joins = ();
foreach my $f(split('\}\s*,\s*{', $config->{'default.EXPORT_JOINS'})) {
    $f =~ s/^\s*\{?\s*//; $f =~ s/\}\s*\}\s*$/}/;
...
my @conditions = ();
foreach my $f(split('\}\s*,\s*{', $config->{'default.EXPORT_CONDITIONS'})) {
    $f =~ s/^\s*\{?\s*//; $f =~ s/\}\s*\}\s*$/}/;
    my ($a, $b) = split('\s*=>\s*{\s*', $f);
    $a =~ s/^\s*\'//; $a =~ s/\'$//g;
    $b =~ s/\s*\}\s*$//;
